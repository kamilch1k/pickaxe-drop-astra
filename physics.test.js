...
test('collision impulses cannot unlock tilt but retain wall-plane torque',()=>{
 for(const sign of [-1,1]){const b=new PickaxeBody(new Vector3(0,30,config.wallCenterZ+sign*config.corridorHalfDepth));b.angularVelocity.set(0,0,0);const r=new Vector3(.2,.3,.1),impulse=new Vector3(3,2,sign*20),expected=r.clone().cross(impulse).multiplyScalar(b.inverseInertia);expected.x=0;expected.y=0;applyImpulse(b,r,impulse);assert.equal(b.velocity.z,0);assert.equal(b.velocity.x,1.5);assert.equal(b.velocity.y,1);assert.ok(b.angularVelocity.distanceTo(expected)<1e-10);}
});
...
test('high depth speeds and repeated side impulses cannot escape the corridor',()=>{
 ...
 for(let i=0;i<120*8;i++){if(i%30===0)applyImpulse(b,new Vector3(.2,.3,0),new Vector3(0,0,i%60===0?100:-100));s.step(config.fixedDt);assert.ok(Math.abs(b.position.z-config.wallCenterZ)<=config.corridorHalfDepth+1e-10);assert.ok(Number.isFinite(b.angularVelocity.length()));assert.equal(b.angularVelocity.x,0);assert.equal(b.angularVelocity.y,0);assert.equal(b.orientation.x,0);assert.equal(b.orientation.y,0);}
...
test('breaking a block rebounds upward instead of drilling through the column',()=>{
 const w=new BlockWorld(),s=new PickaxeSimulator(w),b=new PickaxeBody(new Vector3(.5,config.wallRows+1,0));b.angularVelocity.set(0,0,0);b.velocity.y=-65;s.add(b);
 for(let i=0;i<7&&s.stats.broken===0;i++)s.step(config.fixedDt);
 assert.equal(s.stats.broken,1);assert.ok(b.velocity.y>=config.breakHopSpeed-.2);assert.equal(b.orientation.x,0);assert.equal(b.orientation.y,0);
 const height=b.position.y;for(let i=0;i<12;i++)s.step(config.fixedDt);assert.ok(b.position.y>height);assert.equal(s.stats.broken,1);
});
test('moderate head impacts damage but do not immediately destroy a fresh block',()=>{
 const w=new BlockWorld(),block=w.blocks[0];const damage=.5*config.mass*8**2*config.headDamageMultiplier;
...
test('all spawns and corrupted input orientations are constrained before simulation',()=>{
 ...
 for(let i=0;i<10;i++){const b=new PickaxeBody();assert.equal(b.angularVelocity.x,0);assert.equal(b.angularVelocity.y,0);b.orientation.setFromAxisAngle(new Vector3(1,1,1).normalize(),1.2);b.angularVelocity.set(8,-6,4);s.add(b);assert.equal(b.previousOrientation.x,0);assert.equal(b.previousOrientation.y,0);s.step(config.fixedDt);assert.equal(b.orientation.x,0);assert.equal(b.orientation.y,0);assert.ok(Math.abs(b.angularVelocity.z)>0);}
});
