...
for(const block of world.blocks){if(!block.alive)continue;const hash=Math.abs(Math.sin(block.center.x*127.1+block.center.y*311.7)*43758.5453)%1;const mesh=new T.Mesh(cubeGeometry,palette[Math.floor(hash*palette.length)]);mesh.position.copy(block.center);scene.add(mesh);blocks.set(block,mesh);}
