...
// Fixed front view: the world still has depth and bodies still rotate in 3D.
...
function drop(many=false){for(let i=0;i<(many?8:1);i++){const b=new PickaxeBody(spawnPosition());b.orientation.premultiply(new T.Quaternion().setFromEuler(new T.Euler((Math.random()-.5)*.7,Math.random()*.8,(Math.random()-.5)*1.8)));b.visual=makePickaxe();if(simulator.add(b)){scene.add(b.visual);b.syncVisual();trackedBody=b;camera.position.y=b.position.y-3;}}}
...
function followCamera(dt){
 if(trackedBody){const desired=Math.max(camera.top-1,trackedBody.position.y-camera.top*.25);camera.position.y+=(desired-camera.position.y)*(1-Math.exp(-8*dt));}
 backdrop.position.y=camera.position.y;
}
